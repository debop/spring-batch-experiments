...
/**
 * kr.spring.batch.chapter04.test.incrementer.IncrementerTest
 *
...
public class IncrementerTest {
	...
	public void jobLauncherThenIncrementer() throws Exception {
		...
		assertThat(jobOperator).as("jobOperator must not null").isNotNull();
		...
		assertThat(getJobExecution(idExec3).getExitStatus().getExitCode()).isEqualTo(ExitStatus.FAILED.getExitCode());
		assertThat(getJobInstanceCount()).isEqualTo(3);

		Long idExec4 = jobOperator.startNextInstance(job.getName());
		assertThat(getJobExecution(idExec4).getExitStatus().getExitCode()).isEqualTo(ExitStatus.COMPLETED.getExitCode());
